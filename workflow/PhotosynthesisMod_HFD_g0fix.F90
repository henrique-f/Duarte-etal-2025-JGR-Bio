!Fri Feb 14 04:33:29 PM EST 2025: Mods by Henrique Duarte. For consistency, now applying soil moisture downscalling to the medlynintercept when An>=0. It is already applied when An<0. 
!                                 Changes applied to the ci_func_PHS subroutine.
  ...
  subroutine ci_func_PHS(x,cisun, cisha, fvalsun, fvalsha, p, iv, c, bsun, bsha, bflag, gb_mol, gs0sun, gs0sha,&
    ...
    if ( stomatalcond_mtd == stomatalcond_mtd_medlyn2011 )then
       ...
       if (an_sun(p,iv) >= 0._r8) then
          ...
          aquad = 1.0_r8
       !HFD   bquad = -(2.0_r8 * (medlynintercept(patch%itype(p))*1.e-06_r8 + term) + (medlynslope(patch%itype(p)) * term)**2 / &
       !HFD        (gb_mol*1.e-06_r8 * rh_can))
          bquad = -(2.0_r8 * (max(bsun*medlynintercept(patch%itype(p)),1._r8)*1.e-06_r8 + term) &
           + (medlynslope(patch%itype(p)) * term)**2 / (gb_mol*1.e-06_r8 * rh_can))  !HFD

       !HFD   cquad = medlynintercept(patch%itype(p))*medlynintercept(patch%itype(p))*1.e-12_r8 + &
       !HFD        (2.0_r8*medlynintercept(patch%itype(p))*1.e-06_r8 + term * &
       !HFD        (1.0_r8 - medlynslope(patch%itype(p))* medlynslope(patch%itype(p)) / rh_can)) * term

        cquad = max(bsun*medlynintercept(patch%itype(p)),1._r8)*max(bsun*medlynintercept(patch%itype(p)),1._r8)*1.e-12_r8 + &
               (2.0_r8*max(bsun*medlynintercept(patch%itype(p)),1._r8)*1.e-06_r8 + term * &
               (1.0_r8 - medlynslope(patch%itype(p))* medlynslope(patch%itype(p)) / rh_can)) * term  !HFD
       ...
       !HFD   bquad = -(2.0_r8 * (medlynintercept(patch%itype(p))*1.e-06_r8 + term) + (medlynslope(patch%itype(p)) * term)**2 / &
       !HFD        (gb_mol*1.e-06_r8 * rh_can))
          bquad = -(2.0_r8 * (max(bsha*medlynintercept(patch%itype(p)),1._r8)*1.e-06_r8 + term) &
           + (medlynslope(patch%itype(p)) * term)**2 / (gb_mol*1.e-06_r8 * rh_can))  !HFD

       !HFD   cquad = medlynintercept(patch%itype(p))*medlynintercept(patch%itype(p))*1.e-12_r8 + &
       !HFD        (2.0_r8*medlynintercept(patch%itype(p))*1.e-06_r8 + term * (1.0 - medlynslope(patch%itype(p))* &
       !HFD        medlynslope(patch%itype(p)) / rh_can)) * term

        cquad = max(bsha*medlynintercept(patch%itype(p)),1._r8)*max(bsha*medlynintercept(patch%itype(p)),1._r8)*1.e-12_r8 + &
               (2.0_r8*max(bsha*medlynintercept(patch%itype(p)),1._r8)*1.e-06_r8 + term * &
               (1.0 - medlynslope(patch%itype(p))* medlynslope(patch%itype(p)) / rh_can)) * term  !HFD
